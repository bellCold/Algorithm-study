...
class Solution {
     ...
     * 택배 기사님이 알려준 순서가 기존의 컨테이너 벨트에 네 번째, 세 번째, 첫 번째, 두 번째, 다섯 번째 놓인 택배상자 순서인 경우,(4,3,1,2,5)
    ...
    public int solution(int[] order) {
        ...
        Stack<Integer> assistanceStack = new Stack<>();
        ...
        while (true) {
            if (!mainQueue.isEmpty()) {
                if (order[point] != mainQueue.peek()) {
                    assistanceStack.push(mainQueue.poll());
                } else {
                    mainQueue.poll();
                    answer++;
                    point++;
            ...
            if (!assistanceStack.isEmpty() && assistanceStack.peek() == order[point]) {
                assistanceStack.pop();
                answer++;
                point++;
            }
            if (point >= order.length || (mainQueue.isEmpty() && assistanceStack.peek() != order[point])) {
                break;
            }
        }
